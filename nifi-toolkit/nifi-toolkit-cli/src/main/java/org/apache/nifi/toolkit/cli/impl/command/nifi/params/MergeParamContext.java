...
public class MergeParamContext extends AbstractUpdateParamContextCommand<VoidResult> {
    ...
    public VoidResult doExecute(final NiFiClient client, final Properties properties)
        ...
        final boolean setMissingSensitiveValues = hasArg(properties, CommandOption.SET_MISSING_SENSITIVE_VALUES);
        ...
        for (final ParameterEntity incomingParameterEntity : incomingContext.getParameters()) {
            ...
            if (!existingParameter.isPresent()) {
                final ParameterEntity createdParam = createParameter(incomingParameter, setMissingSensitiveValues);
    ...
    private ParameterEntity createParameter(final ParameterDTO incomingParam, final boolean setMissingSensitiveValues) throws CommandException {
        ...
        if (incomingParam.getSensitive() && incomingParam.getValue() == null) {
            if (setMissingSensitiveValues) {
                printlnIfInteractive("Setting value to empty string for sensitive parameter '" + parameterName + "'");
                newParameter.setValue("");
            } else {
                throw new CommandException("Error processing parameter '" + parameterName
