...
public class QuickImport extends AbstractCompositeCommand<StringResult> {
    ...
    public String getDescription() {
        return "Imports a flow from a file, or a public URL, into a pre-defined bucket named 'QUICK-IMPORT'. This command will " +
    ...
    public StringResult doExecute(final CommandLine cli, final NiFiClient nifiClient, final Properties nifiProps,
        ...
        String registryClientId = null;
        try {
            final StringResult registryClientResult = getRegistryClientId.doExecute(nifiClient, getRegClientProps);
            registryClientId = registryClientResult.getResult();
            if (isInteractive) {
                println();
                println("Found existing registry client...");
            }
        } catch (Exception e) {
            ...
            if (isInteractive) {
                println();
                println("Created new registry client...");
            }
        }
