...
public class PutParquetTest {
    ...
    private void configure(final PutParquet putParquet) throws InitializationException {
        ...
        final RowRecordReaderFactory readerFactory = new AvroReader();
        testRunner.addControllerService("avro-reader-factory", readerFactory);
        testRunner.enableControllerService(readerFactory);

        testRunner.setProperty(PutParquet.RECORD_READER, "avro-reader-factory");
        testRunner.setProperty(PutParquet.DESTINATION_SCHEMA, schema.toString());
    }
    ...
    public void testWriteAvroParquetWithDefaults() throws IOException, InitializationException {
        ...
        final byte[] flowFileContent = createUsers(100);

        testRunner.enqueue(flowFileContent, flowFileAttributes);
        testRunner.run();
    ...
    @Test
    public void testInvalidAvroShouldRouteToFailure() throws InitializationException {
        configure(proc);
        ...
        final byte[] flowFileContent = "NOT AVRO".getBytes(StandardCharsets.UTF_8);
    ...
    public void testCreateDirectoryIOExceptionShouldRouteToRetry() throws InitializationException, IOException {
        ...
        final byte[] flowFileContent = createUsers(10);
    ...
    public void testValidSchemaWithELShouldBeSuccessful() throws InitializationException, IOException {
        configure(proc);
        testRunner.setProperty(PutParquet.DESTINATION_SCHEMA, "${my.schema}");
    ...
    @Test
    public void testMalformedRecordExceptionFromReaderShouldRouteToFailure() throws InitializationException, IOException, MalformedRecordException {
        configure(proc);
        ...
        final RowRecordReaderFactory readerFactory = Mockito.mock(RowRecordReaderFactory.class);
    ...
    public void testIOExceptionCreatingWriterShouldRouteToRetry() throws InitializationException, IOException, MalformedRecordException {
        final PutParquet proc = new PutParquet() {
            @Override
            public HDFSRecordWriter createHDFSRecordWriter(ProcessContext context, FlowFile flowFile, Configuration conf, Path path, String schema)
    ...
    @Test
    public void testIOExceptionFromReaderShouldRouteToRetry() throws InitializationException, IOException, MalformedRecordException {
        configure(proc);

        final RecordReader recordReader = Mockito.mock(RecordReader.class);
        when(recordReader.nextRecord()).thenThrow(new IOException("ERROR"));
    ...
    private byte[] createUsers(final int numUsers) throws IOException {
