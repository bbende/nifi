...
public class PutParquet extends AbstractPutHDFSRecord {
    ...
    public static final AllowableValue AVRO_SCHEMA_TYPE = new AllowableValue("Avro", "Avro", "Avro's schema format");
    public static final AllowableValue[] SCHEMA_TYPES = new AllowableValue[] { AVRO_SCHEMA_TYPE };

    public static final AllowableValue[] COMPRESSION_TYPES;
    static {
        ...
        COMPRESSION_TYPES = compressionTypes.toArray(new AllowableValue[compressionTypes.size()]);
    }

    @Override
    public AllowableValue[] getDestinationSchemaTypes(final ProcessorInitializationContext context) {
        return SCHEMA_TYPES;
    }

    @Override
    public String getDefaultDestinationSchemaType(final ProcessorInitializationContext context) {
        return AVRO_SCHEMA_TYPE.getValue();
    }

    @Override
    public AllowableValue[] getCompressionTypes(final ProcessorInitializationContext context) {
    ...
    @Override
    public HDFSRecordWriter createHDFSRecordWriter(final ProcessContext context, final FlowFile flowFile, final Configuration conf, final Path path, final String schema)
            throws IOException, InvalidSchemaException {
        final String schemaType = context.getProperty(DESTINATION_SCHEMA_TYPE).getValue();
        if (AVRO_SCHEMA_TYPE.getValue().equals(schemaType)) {
            return createAvroParquetHDFSRecordWriter(context, flowFile, conf, path, schema);
        } else {
            throw new IllegalStateException("Unsupported Schema Type: " + schemaType);
        }
    }

    private HDFSRecordWriter createAvroParquetHDFSRecordWriter(final ProcessContext context, final FlowFile flowFile, final Configuration conf, final Path path, final String schema)
            throws IOException, InvalidSchemaException {
        final Schema avroSchema;
        try {
            avroSchema = new Schema.Parser().parse(schema);
        } catch (Exception e) {
            throw new InvalidSchemaException("Unable to parse schema as an Avro schema", e);
        }
    ...
    private void applyCommonConfig(final ParquetWriter.Builder builder, final ProcessContext context, final FlowFile flowFile, final Configuration conf) {
        ...
        final String codecNameValue = context.getProperty(COMPRESSION_TYPE).getValue();
        final CompressionCodecName codecName = CompressionCodecName.valueOf(codecNameValue);
