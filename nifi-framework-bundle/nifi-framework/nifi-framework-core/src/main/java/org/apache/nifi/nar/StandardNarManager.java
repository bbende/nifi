...
public class StandardNarManager implements NarManager, InitializingBean, Closeable {
    ...
    private Collection<NarPersistenceInfo> getExistingNarInfos() throws IOException {
        ...
        if (logger.isDebugEnabled()) {
            logger.debug("NAR Infos before reordering: {}",
                    narInfos.stream()
                            .map(NarPersistenceInfo::getNarProperties)
                            .map(NarProperties::getCoordinate)
                            .toList());
        }
        ...
        final Stack<Collection<NarPersistenceInfo>> narInfoStack = new Stack<>();
        createNarHierarchy(narInfos, narInfoStack);
        ...
        while (!narInfoStack.isEmpty()) {
            final Collection<NarPersistenceInfo> narInfosToAdd = narInfoStack.pop();
            orderedNarInfos.addAll(narInfosToAdd);
        }

        if (logger.isDebugEnabled()) {
            logger.debug("NAR Infos after reordering: {}",
                    orderedNarInfos.stream()
                            .map(NarPersistenceInfo::getNarProperties)
                            .map(NarProperties::getCoordinate)
                            .toList());
        }
    ...
    private void createNarHierarchy(final Collection<NarPersistenceInfo> narInfos, final Stack<Collection<NarPersistenceInfo>> narHierarchy) {
        ...
        // Otherwise remove the parent NARs from the current collection, add the remaining current collection to the hierarchy, and recurse on the parents
        narInfos.removeAll(parentNarInfos);
        narHierarchy.push(narInfos);
    ...
    private synchronized NarNode installNar(final NarInstallRequest installRequest, final File tempNarFile, final boolean async) throws IOException {
        ...
        final Set<Bundle> bundlesWithMatchingDependency = extensionManager.getDependentBundles(coordinate);
        if (!bundlesWithMatchingDependency.isEmpty() && persistenceProvider.exists(coordinate)) {
