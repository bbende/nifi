...
public class StandardUploadRequestReplicator implements UploadRequestReplicator {
    ...
    public StandardUploadRequestReplicator(final ClusterCoordinator clusterCoordinator, final NiFiProperties properties) throws TlsException {
        this.clusterCoordinator = clusterCoordinator;
        this.properties = properties;
        this.webClientService = createClient();
    }

    private WebClientService createClient() throws TlsException {
        ...
        if (properties.isTlsConfigurationPresent()) {
            ...
            final KeyManager[] keyManagers = getKeyManagers(tlsConfig);
            if (keyManagers == null || keyManagers.length == 0) {
                throw new IllegalStateException("Unable to load a KeyManager from the TLS configuration");
            }
    ...
    private KeyManager[] getKeyManagers(final TlsConfiguration tlsConfiguration) throws TlsException {
        ...
        try (InputStream inputStream = new FileInputStream(keystorePath)) {
            ...
            return new StandardKeyManagerFactoryBuilder()
                    .keyStore(keyStore)
